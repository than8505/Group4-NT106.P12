...
namespace sever_tcpchat
{
    public partial class Form1 : Form
    {
        ...
        private void ClientRecv()
        {
            ...
            try
            {
                while (!stopTCPClient && tcpClient.Connected)
                {
                    ...
                    {
                        networkStream.Read(readBuffer, 0, BufferSize);
                    }
                    string headerAndMessage = Encoding.UTF8.GetString(readBuffer).Replace("\0", string.Empty);
                        ...
                        else if (msgType == MessageType.FileEof)
                        {
                            if (string.IsNullOrEmpty(SaveFileName))
                            {
                                ...
                                if (result == DialogResult.Yes)
                                {
                                    ...
                                    DialogSave.RestoreDirectory = true;
                                    DialogSave.Title = "Where do you want to save file?";
                                    DialogSave.InitialDirectory = @"C:/";
        ...
        private void UpdateChatHistoryThreadSafe(string texr)
        {
            if (richtxtPreview.InvokeRequired)
            {
                var d = new SafeCallDelegate(UpdateChatHistoryThreadSafe);
                richtxtPreview.Invoke(d, new object[] { texr });
            }
            else
            {
                richtxtPreview.Text += Text;
            }
        }


        private void richtxtPreview_TextChanged(object sender, EventArgs e)
        {
        ...
        private void btnSendMessage_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string allInOneMsg = $"{txtFriendName.Text}; {MessageType.Text.ToString()};{richtxtInput.Text}";
                byte[] sendingByte = Encoding.UTF8.GetBytes(allInOneMsg);
                ns.Write(sendingByte, 0, sendingByte.Length);
                richtxtInput.Text = "";
            }
            catch(Exception ex)
            {
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.tcpClient.Connect(new IPEndPoint(IPAddress.Parse(txtAddress.Text), serverPort));
            ...
            try
            {
                ...
                using (OpenFileDialog openFileDialog = new OpenFileDialog())
                {
                    ...
                    if (openFileDialog .ShowDialog() == DialogResult.OK)
                    {
                        ...
                        byte[] headerBytes = Encoding.UTF8.GetBytes($"{txtFriendName.Text}; {MessageType.FilePart};");
                        using(Stream fileStream = openFileDialog.OpenFile())
                        {
                            ...
                            for (int i = 0; i < NoOfPackets; i++)
                            {
                                ...
                                else
                                {
                                    ...
                                    headerBytes = Encoding.UTF8.GetBytes($"{txtFriendName.Text}; {MessageType.FileEof};");
                                }
